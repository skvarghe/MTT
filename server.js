...
app.post('/submittime', function (req, res){
  pool.getConnection(function(err,connection){
    ...
    connection.query("INSERT INTO timesheets ('uid','weekending','d1','d2','d3','d4','other','task','hours') values " + req.body.insrt, function (err,rows) {
      console.log("~~~~~~~~~~~~~INSERT INTO timesheets ('uid','weekending','d1','d2','d3','d4','other','task','hours') values " + JSON.stringify(req.body).insrt);
      console.log('~~~~~~~~~~~~~'+JSON.stringify(req.body).insrt);
      if (!err) {
        connection.release();
        resp.appcode="100";
